 ...
 *
 * The library lives on the frame, not on the func, so this has to scan the
 * frame table for a frame belonging to this func. It is meant for one-off
 * lookups, such as answering a single query; don't call it in a loop over
 * funcs. If all you need is the library's name, use getLibNameForFunc instead,
 * which reads it straight off the func's resource.
 */
