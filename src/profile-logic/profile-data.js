 ...
 * enum CallNodeFlag {
 *   UNVISITED = 0,
 *   INSIDE_SELECTED_SUBTREE = 1,
 *   ANCESTOR_OF_SELECTED_NODE = 2,
 *   BEFORE_SELECTED = 3,
 *   AFTER_SELECTED = 4,
 * }
 *
 * The ordering BEFORE_SELECTED / AFTER_SELECTED is determined in the same way
...
function getCallNodeSelectedStates(
  ...
  for (let callNodeIndex = 0; callNodeIndex < callNodeCount; callNodeIndex++) {
    ...
    if (callNodeFlag === 2 /* CallNodeFlag.ANCESTOR_OF_SELECTED_NODE */) {
      ...
      if (callNodeIndex === selectionAncestorAtThisDepth) {
        if (depth === selectedCallNodeDepth) {
          callNodeFlag = 1 /* CallNodeFlag.INSIDE_SELECTED_SUBTREE */;
        } else {
          callNodeFlag = 2 /* CallNodeFlag.ANCESTOR_OF_SELECTED_NODE */;
        }
      } else if (callNodeIndex < selectionAncestorAtThisDepth) {
        callNodeFlag = 3 /* CallNodeFlag.BEFORE_SELECTED */;
      } else {
        callNodeFlag = 4 /* CallNodeFlag.AFTER_SELECTED */;
      }
    } else {
      // All other flags are inherited. For example, if the parent is
      // BEFORE_SELECTED, then so is its entire subtree. Do nothing.
    }
  ...
  for (let sampleIndex = 0; sampleIndex < sampleCount; sampleIndex++) {
    ...
    if (callNodeIndex !== null) {
      switch (callNodeSelectedStates[callNodeIndex]) {
        case 1 /* CallNodeFlags.INSIDE_SELECTED_SUBTREE */:
          sampleSelectedState = 'SELECTED';
          break;
        case 2 /* CallNodeFlags.ANCESTOR_OF_SELECTED_NODE */:
        case 3 /* CallNodeFlags.BEFORE_SELECTED */:
          sampleSelectedState = 'UNSELECTED_ORDERED_BEFORE_SELECTED';
          break;
        case 4 /* CallNodeFlags.AFTER_SELECTED */:
