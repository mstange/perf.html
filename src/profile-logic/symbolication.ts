 ...
 *    profile.libs. The frame's and func's address fields are relative to that
 *    lib. Symbolication groups frames by this field.
 *  - The func's resource field is set to a resource of type "library". The
 *    resource names the library, but it does not identify it: several libs can
 *    share one resource, for example two builds of the same library in a
 *    comparison profile.
 ...
 *
 * This makes a small number of passes over the frame, func and native symbol
 * tables in total, rather than one pass per library. Profiles routinely have
 * hundreds of libraries and millions of frames, so a per-library pass would be
 * far too slow.
 */
...
export function applySymbolicationSteps(
  ...
  const sources = getSourceTableBuilderFromExisting(oldShared.sources);
  const { stringArray } = oldShared;
